...
let volcanoPositions = []; // Array per mouse sopra vulcani
...
function draw() {
  ...
  drawVolcanoes(x, y, mapWidth, mapHeight); drawLegends(); handleVolcanoHover();                              
  
  if (hoveredVolcano) drawTooltip(); 
}
...
function handleVolcanoHover() {
  ...
  for (let volcano of volcanoPositions) {
    if (dist(mouseX, mouseY, volcano.x, volcano.y) <= volcano.size + 3) {
      hoveredVolcano = volcano; cursor(HAND); break;
    }
  }
  if (!hoveredVolcano) cursor(ARROW);
}

function drawTooltip() {
  if (!hoveredVolcano) return;
  
  let tooltipX = mouseX + 15, tooltipY = mouseY - 10, tooltipW = 200, tooltipH = 120;
  
  fill(0, 0, 0, 200); stroke(255, 255, 255, 150); strokeWeight(1);
  rect(tooltipX, tooltipY - tooltipH, tooltipW, tooltipH, 5);
  
  fill(255); noStroke(); textAlign(LEFT, TOP); textStyle(BOLD); textSize(12);
  let textX = tooltipX - 80, textY = tooltipY - tooltipH - 45, lineHeight = 16;
  
  text(hoveredVolcano.name, textX, textY); textY += lineHeight;
  textStyle(NORMAL); textSize(11);
  text("Country: " + hoveredVolcano.country, textX, textY); textY += lineHeight;
  text("Location: " + hoveredVolcano.location, textX, textY); textY += lineHeight;
  text("Elevation: " + hoveredVolcano.elevation + " m", textX, textY); textY += lineHeight;
  text("Type: " + hoveredVolcano.type, textX, textY); textY += lineHeight;
  text("Last Eruption: " + hoveredVolcano.eruption, textX, textY);
}
